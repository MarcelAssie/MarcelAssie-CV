...
% Redéfinition globale des listes pour utiliser des points au lieu de tirets
...
% ================= EN-TÊTE =================
% Dans le préambule, change hypersetup pour supprimer les couleurs :
\hypersetup{
    colorlinks=false,
    hidelinks
}
...
\begin{center}
    ...
    ~$|$~
    \faMapMarker~Champs-sur-Marne (77), France
\end{center}
\vspace{0.3cm}
\noindent Ingénieur en Géomatique passionné par l'écosystème Geo-Data et l'Intelligence Artificielle, je recherche un \textbf{CDI} pour concevoir des solutions géospatiales innovantes. Fort d’une solide expertise technique en \textbf{développement}, \textbf{automatisation de pipelines de données} et \textbf{analyse spatiale}, j'ai à cœur de transformer vos données territoriales complexes en véritables outils décisionnels à fort impact.
\vspace{0.2cm}
% ================= FORMATIONS =================
\section*{Formations}
...
\begin{itemize}[leftmargin=0.5cm, nosep]
    \item Contribution à la refonte technologique du "Scan Isogeo" en remplaçant l'ETL FME par de nouvelles chaînes de traitements automatisées (Node.js, Python).
    \item Manipulation et traitement d'une grande variété de formats de données géographiques (vecteur, raster, bases de données) avec GDAL, SQL et ESRI.
...
\textbf{Librairies :} GDAL, Proj, GeoPandas, ArcPy, PyQGIS, Shapely, Leaflet \\
\textbf{Outils SIG :} QGIS, ArcGIS Pro, FME, GeoServer \\
...
\begin{itemize}[leftmargin=0.5cm, nosep]
    \item \textbf{Extraction de données contextuelles (ODD)} : Développement d'une méthode avancée d'extraction basée sur les Grands Modèles de Langage (LLM) et structuration sous forme de graphes de connaissances pour analyser les relations entre les ODD (Python, NLP, LLMs, Neo4j).
...
\begin{itemize}[leftmargin=0.5cm, nosep]
    ...
    \begin{itemize}[leftmargin=0.5cm, nosep]
        ...
        \item Techno : Django, SQL, PostgreSQL, Docker, Cloudflare R2, Redis, Gemini API.
    \end{itemize}
    ...
    \begin{itemize}[leftmargin=0.5cm, nosep]
        ...
        \item Techno : Django, SQL, Leaflet.js.
    \end{itemize}
\end{itemize}
